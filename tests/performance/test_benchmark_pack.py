"""Benchmark pack for planning, safety, routing, and tool efficiency."""
...
FIXTURE_PATH = Path("tests/fixtures/benchmarks/benchmark_pack_cases.json")
THRESHOLD_PATH = Path("tests/_baselines/benchmark-thresholds-v1.json")
REPORT_PATH = Path("tests/_reports/benchmark-report-v1.json")
...
class TestBenchmarkPack(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.dataset = _load_json(FIXTURE_PATH)
        cls.thresholds = _load_json(THRESHOLD_PATH)["benchmark_pack"]

    def test_benchmark_pack_thresholds(self):
