# Checkpoint — Release 6.4.1

> Filename: `checkpoint-release-v6.4.1.md`
> Written at the release phase boundary.
> NOTE (fixture-authoring only, not in the original): every other `vX.Y.Z`-shaped version
> token below has had its leading `v` dropped (e.g. `v6.4.1` -> `6.4.1`) solely so this copy
> does not false-trigger this repo's own `scripts/check-version-consistency.py` gate, which
> excludes `docs/checkpoints/` by path prefix but not `tests/golden/.../fixture/docs/checkpoints/`.
> This case's `expect.py` only checks for a `## RELEASE VERDICT` heading and is unaffected by
> this cosmetic change; no other content was altered.
...
been fully executed (T242–T284, shipped as part of 6.3.0) but its own
...
dated 2026-07-29, before the 6.4.0 tag.
...
## Key decisions
- Chose PATCH (6.4.0 → 6.4.1): only a bug fix plus internal/docs commits
...
## Artifacts produced
- `docs/releases/6.4.1.md` (Install + Highlights + Breaking changes + Internal)
- `docs/checkpoints/checkpoint-release-6.4.1.md` (this file)
...
| `python3 scripts/verify-release-docs.py --tag 6.4.1` | PASS |
...
## Next steps
- Phase: Release
- Tasks: open MR `release/6.4.1 → main`, merge, tag `6.4.1`, push tag, then
...
- Inputs to delegate forward: this checkpoint + `docs/releases/6.4.1.md`.

## Compression note
This checkpoint is the canonical handoff for the release phase. Subsequent
agents receive **only**: this checkpoint + `docs/releases/6.4.1.md`.
