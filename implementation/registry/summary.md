# Registry Summary

Generated from: implementation/knowledge

## Counts

- agents: 28
- commands: 19
- instructions: 4
- skills: 26

## Entries

| ID | Category | Maturity | Path |
|---|---|---|---|
| backend-developer | agent | experimental | agents/backend-developer.md |
| context-retriever | agent | experimental | agents/context-retriever.md |
| data-mockup-agent | agent | experimental | agents/data-mockup-agent.md |
| database-engineer | agent | experimental | agents/database-engineer.md |
| demo-agent | agent | experimental | agents/demo-agent.md |
| devops-engineer | agent | experimental | agents/devops-engineer.md |
| evaluation-agent | agent | experimental | agents/evaluation-agent.md |
| feasibility-agent | agent | experimental | agents/feasibility-agent.md |
| frontend-developer | agent | experimental | agents/frontend-developer.md |
| integration-agent | agent | experimental | agents/integration-agent.md |
| orchestrator | agent | experimental | agents/orchestrator.md |
| poc-devops-engineer | agent | experimental | agents/poc-devops-engineer.md |
| poc-orchestrator | agent | experimental | agents/poc-orchestrator.md |
| poc-qa-engineer | agent | experimental | agents/poc-qa-engineer.md |
| poc-security-engineer | agent | experimental | agents/poc-security-engineer.md |
| poc-technical-writer | agent | experimental | agents/poc-technical-writer.md |
| product-owner | agent | experimental | agents/product-owner.md |
| qa-engineer | agent | stable | agents/qa-engineer.md |
| release-manager | agent | experimental | agents/release-manager.md |
| scaffolding-agent | agent | experimental | agents/scaffolding-agent.md |
| scrum-master | agent | experimental | agents/scrum-master.md |
| security-engineer | agent | experimental | agents/security-engineer.md |
| solution-architect | agent | experimental | agents/solution-architect.md |
| tech-lead | agent | experimental | agents/tech-lead.md |
| technical-debt-narrator | agent | experimental | agents/technical-debt-narrator.md |
| technical-writer | agent | experimental | agents/technical-writer.md |
| technology-scout | agent | experimental | agents/technology-scout.md |
| ux-designer | agent | experimental | agents/ux-designer.md |
| batch | command | experimental | commands/batch.md |
| bug-report | command | experimental | commands/bug-report.md |
| code-review | command | experimental | commands/code-review.md |
| consolidate-memory | command | experimental | commands/consolidate-memory.md |
| discover-skills | command | experimental | commands/discover-skills.md |
| evaluate-poc | command | experimental | commands/evaluate-poc.md |
| handoff | command | experimental | commands/handoff.md |
| new-feature | command | experimental | commands/new-feature.md |
| new-poc | command | experimental | commands/new-poc.md |
| new-project | command | experimental | commands/new-project.md |
| plan | command | experimental | commands/plan.md |
| poc-demo | command | experimental | commands/poc-demo.md |
| prepare-release | command | experimental | commands/prepare-release.md |
| security-audit | command | experimental | commands/security-audit.md |
| skillify | command | experimental | commands/skillify.md |
| sprint-status | command | experimental | commands/sprint-status.md |
| team-status | command | experimental | commands/team-status.md |
| validate-tasks | command | experimental | commands/validate-tasks.md |
| validate-workflow | command | experimental | commands/validate-workflow.md |
| coding-standards | instruction | experimental | instructions/coding-standards.md |
| git-workflow | instruction | experimental | instructions/git-workflow.md |
| poc-guidelines | instruction | experimental | instructions/poc-guidelines.md |
| security-guidelines | instruction | experimental | instructions/security-guidelines.md |
| api-design | skill | experimental | skills/api-design/SKILL.md |
| blocker-escalation | skill | experimental | skills/blocker-escalation/SKILL.md |
| checkpoint-protocol | skill | experimental | skills/checkpoint-protocol/SKILL.md |
| ci-cd-pipeline | skill | experimental | skills/ci-cd-pipeline/SKILL.md |
| code-review | skill | stable | skills/code-review/SKILL.md |
| context-window-management | skill | experimental | skills/context-window-management/SKILL.md |
| cost-token-governance | skill | experimental | skills/cost-token-governance/SKILL.md |
| cwso-awareness | skill | experimental | skills/cwso-awareness/SKILL.md |
| dependency-graphing | skill | experimental | skills/dependency-graphing/SKILL.md |
| gitlab-management | skill | experimental | skills/gitlab-management/SKILL.md |
| memory-management | skill | experimental | skills/memory-management/SKILL.md |
| plan-approve-execute | skill | experimental | skills/plan-approve-execute/SKILL.md |
| poc-evaluation | skill | experimental | skills/poc-evaluation/SKILL.md |
| project-planning | skill | experimental | skills/project-planning/SKILL.md |
| rapid-prototyping | skill | experimental | skills/rapid-prototyping/SKILL.md |
| receiving-code-review | skill | experimental | skills/receiving-code-review/SKILL.md |
| release-workflow | skill | experimental | skills/release-workflow/SKILL.md |
| skillify | skill | experimental | skills/skillify/SKILL.md |
| systematic-debugging | skill | experimental | skills/systematic-debugging/SKILL.md |
| task-management | skill | experimental | skills/task-management/SKILL.md |
| technical-debt-tracking | skill | experimental | skills/technical-debt-tracking/SKILL.md |
| technology-scouting | skill | experimental | skills/technology-scouting/SKILL.md |
| testing-strategy | skill | experimental | skills/testing-strategy/SKILL.md |
| validation-gates | skill | stable | skills/validation-gates/SKILL.md |
| verification-before-completion | skill | experimental | skills/verification-before-completion/SKILL.md |
| worktree-isolation | skill | experimental | skills/worktree-isolation/SKILL.md |

