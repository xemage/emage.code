# v3 Registry Summary

Generated from: implementation/knowledge

## Counts

- agents: 27
- commands: 18
- instructions: 4
- skills: 25

## Entries

| ID | Category | Maturity | Path |
|---|---|---|---|
| backend-developer | agent | beta | agents/backend-developer.md |
| data-mockup-agent | agent | beta | agents/data-mockup-agent.md |
| database-engineer | agent | beta | agents/database-engineer.md |
| demo-agent | agent | beta | agents/demo-agent.md |
| devops-engineer | agent | beta | agents/devops-engineer.md |
| evaluation-agent | agent | beta | agents/evaluation-agent.md |
| feasibility-agent | agent | beta | agents/feasibility-agent.md |
| frontend-developer | agent | beta | agents/frontend-developer.md |
| integration-agent | agent | beta | agents/integration-agent.md |
| orchestrator | agent | beta | agents/orchestrator.md |
| poc-devops-engineer | agent | beta | agents/poc-devops-engineer.md |
| poc-orchestrator | agent | beta | agents/poc-orchestrator.md |
| poc-qa-engineer | agent | beta | agents/poc-qa-engineer.md |
| poc-security-engineer | agent | beta | agents/poc-security-engineer.md |
| poc-technical-writer | agent | beta | agents/poc-technical-writer.md |
| product-owner | agent | beta | agents/product-owner.md |
| qa-engineer | agent | beta | agents/qa-engineer.md |
| release-manager | agent | beta | agents/release-manager.md |
| scaffolding-agent | agent | beta | agents/scaffolding-agent.md |
| scrum-master | agent | beta | agents/scrum-master.md |
| security-engineer | agent | beta | agents/security-engineer.md |
| solution-architect | agent | beta | agents/solution-architect.md |
| tech-lead | agent | beta | agents/tech-lead.md |
| technical-debt-narrator | agent | beta | agents/technical-debt-narrator.md |
| technical-writer | agent | beta | agents/technical-writer.md |
| technology-scout | agent | beta | agents/technology-scout.md |
| ux-designer | agent | beta | agents/ux-designer.md |
| batch | command | beta | commands/batch.md |
| bug-report | command | beta | commands/bug-report.md |
| code-review | command | beta | commands/code-review.md |
| consolidate-memory | command | beta | commands/consolidate-memory.md |
| discover-skills | command | beta | commands/discover-skills.md |
| evaluate-poc | command | beta | commands/evaluate-poc.md |
| handoff | command | beta | commands/handoff.md |
| new-feature | command | beta | commands/new-feature.md |
| new-poc | command | beta | commands/new-poc.md |
| new-project | command | beta | commands/new-project.md |
| plan | command | beta | commands/plan.md |
| poc-demo | command | beta | commands/poc-demo.md |
| prepare-release | command | beta | commands/prepare-release.md |
| security-audit | command | beta | commands/security-audit.md |
| skillify | command | beta | commands/skillify.md |
| sprint-status | command | beta | commands/sprint-status.md |
| team-status | command | beta | commands/team-status.md |
| validate-workflow | command | beta | commands/validate-workflow.md |
| coding-standards | instruction | beta | instructions/coding-standards.md |
| git-workflow | instruction | beta | instructions/git-workflow.md |
| poc-guidelines | instruction | beta | instructions/poc-guidelines.md |
| security-guidelines | instruction | beta | instructions/security-guidelines.md |
| api-design | skill | beta | skills/api-design/SKILL.md |
| blocker-escalation | skill | beta | skills/blocker-escalation/SKILL.md |
| checkpoint-protocol | skill | beta | skills/checkpoint-protocol/SKILL.md |
| ci-cd-pipeline | skill | beta | skills/ci-cd-pipeline/SKILL.md |
| code-review | skill | beta | skills/code-review/SKILL.md |
| context-window-management | skill | beta | skills/context-window-management/SKILL.md |
| cost-token-governance | skill | beta | skills/cost-token-governance/SKILL.md |
| dependency-graphing | skill | beta | skills/dependency-graphing/SKILL.md |
| gitlab-management | skill | beta | skills/gitlab-management/SKILL.md |
| memory-management | skill | beta | skills/memory-management/SKILL.md |
| plan-approve-execute | skill | beta | skills/plan-approve-execute/SKILL.md |
| poc-evaluation | skill | beta | skills/poc-evaluation/SKILL.md |
| project-planning | skill | beta | skills/project-planning/SKILL.md |
| rapid-prototyping | skill | beta | skills/rapid-prototyping/SKILL.md |
| receiving-code-review | skill | beta | skills/receiving-code-review/SKILL.md |
| release-workflow | skill | beta | skills/release-workflow/SKILL.md |
| skillify | skill | beta | skills/skillify/SKILL.md |
| systematic-debugging | skill | beta | skills/systematic-debugging/SKILL.md |
| task-management | skill | beta | skills/task-management/SKILL.md |
| technical-debt-tracking | skill | beta | skills/technical-debt-tracking/SKILL.md |
| technology-scouting | skill | beta | skills/technology-scouting/SKILL.md |
| testing-strategy | skill | beta | skills/testing-strategy/SKILL.md |
| validation-gates | skill | beta | skills/validation-gates/SKILL.md |
| verification-before-completion | skill | beta | skills/verification-before-completion/SKILL.md |
| worktree-isolation | skill | beta | skills/worktree-isolation/SKILL.md |

