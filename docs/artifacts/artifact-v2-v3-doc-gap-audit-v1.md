...
- v3 implementation documentation: `implementation/README.md`
- v3 entry scripts: `implementation/scripts/verify-v3.mjs`, `implementation/scripts/check-v3.py`
...
| G003 | Medium | The v3 runbook documents a drift-check command that is not valid from the stated working directory. | `implementation/README.md` currently shows `node implementation/scripts/verify-v3.mjs --root ../../v2/implementation`, but that path does not resolve as documented. | Fix either the command example or the script path resolution so the documented invocation works. |
