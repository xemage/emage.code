...
**Picked up by:** devops-engineer
**Date:** 2026-08-07

`scripts/check-main-develop-drift.py` was created with the exact content specified in this brief
(no syntax fixes were needed — the file as given is valid Python).

### AC1 — file exists with exact content

Created verbatim from the brief's "Exact file content" block. No changes made.

### AC2 — `python3 -c "import ast; ast.parse(...)"` exits 0

```
$ python3 -c "import ast; ast.parse(open('scripts/check-main-develop-drift.py').read())"; echo "AST_EXIT=$?"
AST_EXIT=0
```

### AC3 — run against real repo state
...
Matches the expected output noted in the brief: `main` and `develop` are both at `v6.5.0`, 0
releases behind, PASS, exit code 0.

### AC4 — executable bit set

```
$ chmod +x scripts/check-main-develop-drift.py && ls -l scripts/check-main-develop-drift.py
-rwxr-xr-x 1 emage emage 5344 Aug  7 10:30 scripts/check-main-develop-drift.py
```

### Summary

All 4 acceptance criteria pass. Only `scripts/check-main-develop-drift.py` was created and this
file (`docs/tasks/task-T332.md`) was updated; no other files were touched. No git commit was made
(orchestrator handles commits per instructions).
